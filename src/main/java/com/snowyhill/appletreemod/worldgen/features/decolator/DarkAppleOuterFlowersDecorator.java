 ...
 */
public class DarkAppleOuterFlowersDecorator extends TreeDecorator {

    // JSONでも使えるよう、確率(outer_chance)を追加（既定=0.5）
    public static final MapCodec<DarkAppleOuterFlowersDecorator> CODEC =
            ...
            ).apply(inst, DarkAppleOuterFlowersDecorator::new));
    ...
    protected TreeDecoratorType<?> type() {
        return ModTreeDecorators.DARK_APPLE_OUTER_FLOWERS.get();
    }
    ...
    public void place(Context ctx) {
        ...
        final BlockState flower = ModBlocks.DARK_APPLE_FLOWER_LEAVES.get().defaultBlockState();

        for (BlockPos pos : leaves) {
            // 置換対象は APPLE_LEAVES のみ（既に花葉は触らない）
            boolean isAppleLeaves = level.isStateAtPosition(pos, s -> s.is(ModBlocks.DARK_APPLE_LEAVES.get()));
